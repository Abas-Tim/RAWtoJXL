...
xUnit v3 test suite for RAWtoJXL.Core with DI-based integration tests and unit tests.
...
- Depends on `RAWtoJXL.Core` for services under test
- Depends on `RAWtoJXL.Avalonia` for GUI test target application
