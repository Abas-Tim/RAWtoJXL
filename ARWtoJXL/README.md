...
A high-performance Windows desktop application built with .NET 8 and WPF to convert Sony RAW (.ARW) files to JPEG-XL (.JXL) format.
...
```powershell
dotnet publish ARWtoJXL/ARWtoJXL.WPF/ARWtoJXL.WPF.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -p:PublishTrimmed=true
```

## Development
- **Testing:** Run `dotnet test` to execute the xUnit test suite.
- **Tech Stack:** .NET 8, WPF, MVVM (CommunityToolkit), Magick.NET.
